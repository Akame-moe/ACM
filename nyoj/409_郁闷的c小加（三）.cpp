...
int main()
{
    ...
    int n,m;
    printf("\n\n+++++++++++++++欢迎进入本程序+++++++++++++++\n\n\n\n");
    printf("请输入中缀表达式(式子以\"=\"结束):");scanf("%s",s);
    do{
        printf("\n+++++++++++++++请选择你的操作++++++++++++\n");
        ...
        switch(n)
        {
        case 1:printf("\n\n%s%.2lf",s,work(s));break;
        case 2:printf("前缀式是: ");_visit(creat_tree(s));printf("=\n");break;
        case 3:printf("后缀式是: ");visit(creat_tree(s));printf("=\n");break;
        case 4:printf("请输入中缀表达式(式子以\"=\"结束):");scanf("%s",s);break;
        case 5:system("cls");break;
        case 6:printf("确定要退出？(1/0):");scanf("%d",&m);if(m) exit(0); else break;
        }
    }while(1);
    return 0;
...
void visit(tree * r)
{
    if(r!=NULL)
    {
        ...
        { printf("%c ",r->c); r->n=calculate(r->liftchild->n,r->c,r->rightchild->n);}//遍历的时候计算。
    }
}
void _visit(tree * r)
{
...
tree* creat_tree(char *s)
{
    ...
    for(i=0;s[i]!='=';)
    {
        ...
        else
        {
            ...
            {
                ops1.top()->rightchild=ovs1.top();ovs1.pop();
                ops1.top()->liftchild=ovs1.top();ovs1.pop();                    
                Root=ops1.top();ops1.pop();
                ovs1.push(Root);
            }
        }
    }
